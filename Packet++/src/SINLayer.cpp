...
SINLayer::SINLayer(const uint8_t command, const uint8_t num_entries, const uint8_t* keys) : Layer()
{
...
std::string SINLayer::toString() {
	char TS[500];
	sprintf(TS, "SIN=> Command:%X, Num_Entries: %d, Keys=", getCommand(), getNumEntries());
